...
function makeRing(capacity) {
  return {
    ...
    toArray() {
      ...
      for (let i = 0; i < n; i++) {
        arr.push({ t: this.times[idx], dt: this.dts[idx] });
...
export function computeDroppedFrames(dt, vsyncMs = 16.667) {
  const v = vsyncMs;
...
export function createMonitor({ windowMs = 5000, notifyMs = 250, histogramEdgesMs } = {}) {
  ...
  function makeSnapshot(t, ring, edges, windowMs, vsync, lifetimeDropped, lifetimeFrames) {
    ...
    let windowDropped = 0;
    for (const dt of dts) windowDropped += computeDroppedFrames(dt, vsync);
    return {
      now: t,
      windowMs,
      samples: ring.count,
      lastDtMs: dts.length ? dts.at(-1) : 0,
      avgFps,
      minFps,
      maxFps,
  ...
  const tick = (now) => {
    if (running) {
      ...
      if (lastT) {
        ...
        const due = lastNotify === 0 || (t - lastNotify) >= notifyMs;
        if (due) {
          lastNotify = t;
          if (onSnapshot) {
            const snap = makeSnapshot(t, ring, edges, windowMs, vsync, lifetimeDropped, lifetimeFrames);
            onSnapshot(snap);
          }
        }
      }
